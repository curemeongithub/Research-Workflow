...
%   1. Replace [YOUR NAME] and [YOUR INSTITUTION] in the title block.
...
\title[Convex ReLU Networks]{%
  ...
  \normalsize The Non-Convex Mystery, Solved?%
}
  ...
  \begin{columns}[c]
    \begin{column}{0.5\textwidth}
      \begin{itemize}
        ...
        \item The folk answer: \textit{``overparameterization, good init, luck''}
        \item \textbf{A better answer exists.\ It starts in 2020.}
      \end{itemize}
    \end{column}
    \begin{column}{0.48\textwidth}
      % FIGURE DESCRIPTION: Split image. Left half: cartoon non-convex loss
      % landscape (wavy 2D surface, several local minima as dots), caption
      % "non-convex by construction." Right half: smooth convex bowl with a
      % single global minimum, caption "convex at the global optimum."
      % Double-headed arrow between them labelled "?".
      % Draw as TikZ or replace with an external figure.
      % TO REPLACE: Save your figure as figures/slide2_mystery.pdf and use:
      %   \includegraphics[width=\textwidth]{figures/slide2_mystery.pdf}
      \begin{center}
        \begin{tikzpicture}[scale=0.55]
          % Non-convex surface (left)
          \draw[thick, red!60!black, domain=0:3.2, samples=60]
            plot (\x, {0.7*sin(3*\x r) + 0.4*sin(5*\x r) + 1.5});
          \node[red!60!black, font=\tiny] at (1.6, 0.2) {non-convex};
          \fill[red!60!black] (0.55,1.47) circle (2pt)
                               (1.55,1.05) circle (2pt)
                               (2.55,1.63) circle (2pt);
          % Convex bowl (right)
          \draw[thick, blue!70!black, domain=3.8:7.0, samples=60]
            plot (\x, {0.6*(\x-5.4)^2 + 0.5});
          \fill[blue!70!black] (5.4,0.5) circle (2pt);
          \node[blue!70!black, font=\tiny] at (5.4, 0.1) {convex};
          % Arrow
          \draw[<->, thick, gray] (3.3, 1.5) -- (3.7, 1.5)
...
%
\begin{frame}{Duality Gap in 45 Seconds}
  \begin{center}
    \vspace{0.4em}
  ...
  \begin{block}{}
    \centering
    For non-convex problems this almost never holds\ldots\ \textbf{almost.}
  \end{block}
\end{frame}
...
%
\begin{frame}{The Bridge Is Built: \citeauthor{arxiv2002.10553} (\citeyear{arxiv2002.10553})}
  ...
  \vspace{0.3em}
  \begin{alertblock}{Theorem 1 \normalfont\small[\citeauthor{arxiv2002.10553}, \citeyear{arxiv2002.10553}]}
    Two-layer ReLU $+$ weight decay is \textbf{secretly convex.}
...
%
\begin{frame}{The Climax: Depth Breaks, Parallelism Restores}
  ...
  \vspace{0.4em}
  \begin{center}
    \large\textbf{Architecture, not depth, is the villain.}\\
...
%
\begin{frame}{The Theory Shipped}

  \begin{center}
    \textbf{\large The theory shipped.}
  \end{center}
  \vspace{-0.3em}

  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \textbf{Theory:} \citeauthor{arxiv2402.03625} (\citeyear{arxiv2402.03625})
      \vspace{0.3em}

      % FIGURE DESCRIPTION: Two progress bars. Grey bar (wider): "Exact reformulation
      % O((n/r)^r) variables". Blue bar (shorter): "Random subsample O(log n) patterns".
      % TO REPLACE: use figures/slide6_theory_bars.pdf
      \begin{tikzpicture}[scale=0.9]
        % Exact bar (grey, wide)
        \fill[gray!40] (0,0.5) rectangle (3.8,0.9);
        \node[right, font=\tiny] at (3.85,0.7) {exact: $O\!\left((\nn/\rrank)^{\rrank}\right)$};
        % Approx bar (blue, short)
        \fill[blue!60] (0,0) rectangle (1.4,0.4);
        \node[right, font=\tiny] at (1.45,0.2) {random: $O(\log\nn)$ patterns};
      \end{tikzpicture}

      \vspace{0.6em}
      \textbf{Approximation guarantee (Theorem 2.1):}
      \[
        \Pstar \;\leq\; \ptildestar \;\leq\; \CC\sqrt{\log 2\nn}\;\Pstar
      \]
      \hspace*{0.5em}\textit{\tiny First poly-time approx.\ guarantee for regularized ReLU NNs.}
    \end{column}
    \hfill
    \begin{column}{0.48\textwidth}
      \textbf{Practice:} \citeauthor{user8652CRONOS} (\citeyear{user8652CRONOS})
      \vspace{0.3em}

      % FIGURE DESCRIPTION: Bar chart with two bars: "Adam (tuned)" vs "CRONOS-AM"
      % on ImageNet, CRONOS matching or slightly taller. Tags: JAX, RTX-4090, ADMM, ImageNet.
      % TO REPLACE: use figures/slide6_cronos_bars.pdf
      \begin{tikzpicture}[scale=0.9]
        \fill[gray!50]  (0,0) rectangle (0.6,2.2);
        \fill[blue!60]  (0.9,0) rectangle (1.5,2.3);
        \node[below, font=\tiny] at (0.3,0)  {Adam};
        \node[below, font=\tiny] at (1.2,0)  {CRONOS-AM};
        \node[above, font=\tiny] at (0.3,2.2) {\small$\approx$};
        \node[above, font=\tiny] at (1.2,2.3) {\small$\approx$};
        \draw[->] (0,0) -- (0,2.7) node[above, font=\tiny] {acc.};
        \node[font=\tiny, align=left] at (2.5,1.2) {JAX\\ RTX-4090\\ ADMM\\ ImageNet};
      \end{tikzpicture}
  ...
  % Theorem 2.3 (line 142), Remark 2.2 (line 133).
  % CRONOS: Feng, Frangella & Pilanci 2023 (user-8652-CRONOS, lines 26, 40).
\end{frame}


% ============================================================
% ACT 4: OPEN QUESTIONS AND CLOSE (target: 1:00, Slides 7-8)
% ============================================================

% ---- SLIDE 7: OPEN QUESTION --- WHAT IS A "CONVEX ReLU NETWORK"? (7:00-7:40, 40 sec) ----
% SPEAKING TIME: ~40 seconds (one open question, one external group, one sentence)
% TIMING CHECK: Begin close at 7:00. Slide 7 ends at 7:40.
%
% SPEAKING NOTES:
% Distinguish the two questions:
% "I have been talking about convexifying the training problem. There is a
%  different question -- which ReLU networks *represent* a convex function as
%  a map from input to output? This matters when you need the network itself
%  to be convex, as in optimal transport or energy models."
%
% Deliver the open question:
% "The standard answer has been Input-Convex Neural Networks. In 2025 Gagneux
%  and colleagues showed that from two hidden layers onward, there exist convex
%  functions a ReLU network can express that no ICNN of the same architecture
%  can. The right convex class is still unknown."
%
% Acknowledge the external provenance:
% "This is from a group outside Stanford, which I think is worth noting for a
%  survey talk."
%
% BRIDGE TO SLIDE 8: "So to summarise where the field stands..."
%
% IF RUNNING SHORT (need to cut): Fold Slide 7 into a single sentence at the
% start of Slide 8: "One last thing -- Gagneux et al. 2025 (non-Stanford group)
% show the right convex architectural class for ReLU networks is still open."
%
\begin{frame}{Open Question: What Is a ``Convex ReLU Network''?}

  \begin{columns}[c]
    \begin{column}{0.52\textwidth}
      \begin{itemize}
        \item Training is convex. But which \textit{networks} represent convex functions?
        \item Standard answer: \textbf{ICNNs} (Amos et al.\ 2017)
        \item \citeauthor{arxiv2501.03017} (\citeyear{arxiv2501.03017}):\\
              at depth $\geq 2$,\quad ICNN $\subsetneq$ ReLU-convex
        \item The right convex class is \textbf{still open}
        \item Matters for: optimal transport, energy-based models
      \end{itemize}
      \vspace{0.5em}
      \textit{\scriptsize Non-Stanford group: INRIA / Univ.\ de Lyon.}
    \end{column}
    \begin{column}{0.44\textwidth}
      % FIGURE DESCRIPTION: Venn diagram. Large outer ellipse: "Convex functions
      % a 2-layer ReLU network can represent." Smaller inner ellipse: "Convex
      % functions an ICNN of the same architecture can represent." Shaded crescent
      % with "?" label: "unknown since depth >= 2."
      % TO REPLACE: use figures/slide7_venn.pdf
      \begin{center}
        \begin{tikzpicture}
          % Outer ellipse
          \draw[thick, blue!70] (0,0) ellipse (1.8cm and 1.1cm);
          \node[font=\tiny, blue!70, align=center] at (0, 1.3)
            {Convex fns.\ a ReLU net\\ can represent};
          % Inner ellipse
          \fill[green!20, opacity=0.7] (-0.4,0) ellipse (0.9cm and 0.65cm);
          \draw[thick, green!60!black] (-0.4,0) ellipse (0.9cm and 0.65cm);
          \node[font=\tiny, green!60!black, align=center] at (-0.4, 0)
            {ICNNs};
          % Crescent question mark
          \node[font=\Large, red!70!black] at (1.1, 0) {\textbf{?}};
          \node[font=\tiny, red!70!black] at (1.1, -0.35) {unknown};
        \end{tikzpicture}
      \end{center}
    \end{column}
  \end{columns}

  % SOURCE: Gagneux, Massias, Soubies & Gribonval 2025 (arxiv-2501.03017, line 1261)
  % explicitly leaves deep-ReLU convex-class characterisation as future work.
\end{frame}


% ---- SLIDE 8: TAKEAWAY (7:40-8:00, 20 seconds) ----
...
\begin{frame}{Takeaway}
  ...
  \begin{center}
    ...
    \Large\textbf{width \quad|\quad architecture \quad|\quad $\log n$}
  \end{center}
