...
**All sources are stored in `sources/` — NOT in each chapter folder.**
...
| **Blog posts, Substack, Medium, JS-heavy pages** | `.venv/bin/python scripts/authenticated_extract.py "URL"` (add `--profile substack` or `--profile medium` for login-gated sites) |
| **d2l.ai chapters** | `.venv/bin/python scripts/authenticated_extract.py "https://d2l.ai/{chapter}/{section}.html" -s ".document"` |
| **Static HTML pages (fast fallback, no JS)** | `.venv/bin/python scripts/webpage_to_md.py "URL" -o "sources/{domain}/{path}/"` |
...
Many arXiv papers include figures as PDFs. Markdown cannot embed PDFs inline, so they must be converted to PNG.
...
Each section file should include its own sources as a collapsible header at the top. See `markdown-conventions.md` (Per-Section Source Headers section) for the full template.

<!-- IGNORE.
## Blog Attribution

Many sources come from independent researchers' blogs. These must be attributed in source headers, figure captions, and in-text framings. See `high-quality-blogs.md` for the curated blog registry, and `markdown-conventions.md` (Attribution for Blog Content) for the formatting rules. -->
