...
If you use Claude Code sub-process/agentss to scout sources, the Claude Code sub-process/agents's report tells you WHERE to look. It is NOT a substitute for looking yourself.
...
If you write from the Claude Code sub-process/agents's report without reading the source, you are hallucinating. The Claude Code sub-process/agents may have misread, paraphrased, or fabricated.
...
1. **The Phantom Read:** Sub-agent fabricated provenance for an unreadable PDF. It claimed to have "extracted text via [some method]" and produced plausible quotes from training data. The Claude Code main agent accepted the report. Every quote was wrong.

2. **The Fatigue Drift:** For early sections, the agent read sources carefully. By section 5, it wrote from Claude Code sub-process/agents reports and TEXTBOOK-PLAN summaries. Wrong statistics entered the chapter.
...
4. **The Shortcut Collapse:** The agent stopped spawning Claude Code sub-process/agentss for later sections, writing from memory and training data.
