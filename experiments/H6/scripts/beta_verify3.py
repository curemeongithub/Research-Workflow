"""
Fast beta formula verification.
n=50, d=2, width=2, large lambda_2=1.0 (very heavy reg -> clear underparameterized + converges fast).
"""
import numpy as np
import cvxpy as cp
import torch
...
n, d, width = 50, 2, 2
lambda_2 = 1.0  # very heavy reg -> small weights -> easy to converge
...
for restart in range(100):
    ...
    scheduler = torch.optim.lr_scheduler.MultiStepLR(opt, milestones=[2000, 5000], gamma=0.1)
...
for beta_name, beta in [
    ('lambda_2=1.0', lambda_2),
    ('2*lambda_2=2.0', 2*lambda_2),
    ('2*sqrt(lambda_2)=2.0', 2*np.sqrt(lambda_2)),
]:
    ...
    prob.solve(solver=cp.SCS, eps=1e-10, max_iters=500000, verbose=False)
    ...
    print(f"\nConvex (beta={beta_name}, P={P}, status={prob.status}):")
    ...
    print(f"  rel_diff: {abs(float(prob.value) - best_loss)/max(best_loss,1e-12)*100:.4f}%")
    sys.stdout.flush()
